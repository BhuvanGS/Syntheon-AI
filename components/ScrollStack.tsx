...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const getElementOffset = useCallback(
    (element: HTMLElement, index?: number) => {
      if (useWindowScroll && index !== undefined && cardOffsetsRef.current[index] !== undefined) {
        return cardOffsetsRef.current[index];
      }
      if (
        useWindowScroll &&
        element.classList?.contains('scroll-stack-end') &&
        endOffsetRef.current
      ) {
        return endOffsetRef.current;
      }
      if (useWindowScroll) {
  ...
  useLayoutEffect(() => {
    ...
    const nativeScrollHandler = () => handleScroll();
    if (scrollerEl) {
      scrollerEl.addEventListener('scroll', nativeScrollHandler, { passive: true });
    }
    ...
    return () => {
      ...
      if (scrollerEl) {
        scrollerEl.removeEventListener('scroll', nativeScrollHandler);
      }
      stackCompletedRef.current = false;
      cardsRef.current = [];
